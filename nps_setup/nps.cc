...
int main(int argc,char** argv)
{
  ...
  runManager->SetUserInitialization(new NPSActionInitialization());

  // Set a HistoManager.
  // Look for root filename in the input line, otherwise do the default.

  char *rootFileName="nps_setup.root";
  if ( argc >= 3 )    rootFileName=argv[2];

  NPSHistoManager*  histo = new NPSHistoManager(rootFileName);

  // Set user action classes.

  NPSPrimaryGeneratorAction* gen_action = new NPSPrimaryGeneratorAction();
  runManager->SetUserAction(gen_action);

  NPSRunAction* run_action = new NPSRunAction(histo);  
  runManager->SetUserAction(run_action);

  NPSEventAction* event_action = new NPSEventAction(histo);
  runManager->SetUserAction(event_action);

  NPSSteppingAction* stepping_action = new NPSSteppingAction(event_action);
